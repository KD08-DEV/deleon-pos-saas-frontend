...
const CashRegister = () => {
    ...
    // Debounce de filtros del modal
    const [debouncedModalFilters, setDebouncedModalFilters] = useState(modalFilters);
    useEffect(() => {
        const id = setTimeout(() => setDebouncedModalFilters(modalFilters), 400);
    ...
    const modalFilteredReports = useMemo(() => {
        const from = debouncedModalFilters.from ? new Date(`${debouncedModalFilters.from}T00:00:00`) : null;
        const to = debouncedModalFilters.to ? new Date(`${debouncedModalFilters.to}T23:59:59`) : null;
        const method = normalize(debouncedModalFilters.method);
        const user = normalize(debouncedModalFilters.user);
        const client = normalize(debouncedModalFilters.client);
    ...
    }, [sortedReports, debouncedModalFilters]);

    // Resumen basado en los últimos 10 registros para la vista inicial
    ...
    const modalCashClosure = useMemo(() => {
        ...
        for (const r of modalFilteredReports) {
            const total = Number(r?.bills?.totalWithTax || 0);
            const pmRaw = normalizeMethod(r?.paymentMethod);
    ...
    const downloadExcel = async (reportsToExport = recentReports) => {
        try {
    ...
    return (
        <div className={showFullView ? "pointer-events-none select-none" : ""}>
            ...
            {/* Resumen de cierre de caja (vista inicial - últimos 10) */}
            <div className="mb-6 rounded-lg border border-gray-800/50 bg-gradient-to-br from-[#111111] to-[#0a0a0a] p-5">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-sm text-gray-300">
                        Total: <span className="font-semibold text-[#f6b100] text-lg">{currency(initialCashClosure.grandTotal)}</span>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    {Object.entries(initialCashClosure.buckets).map(([k, v]) => (
                        <div key={k} className="rounded-lg bg-[#1a1a1a] border border-gray-800/30 p-3 hover:border-[#f6b100]/30 transition-colors">
            ...
            <div className="flex gap-3 mb-6">
                <button
                    onClick={() => downloadExcel(recentReports)}
            ...
            {/* Tabla de registros inicial (últimos 10) */}
            {isLoading ? (
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody>
                                {recentReports.length === 0 ? (
                                    ...
                                    recentReports.map((r) => (
                                        <tr key={r._id} className="border-b border-gray-800/30 hover:bg-[#1a1a1a]/50 transition-colors">
            ...
            {/* Modal flotante de registros completos */}
            {showFullView && (
                <div 
                    ...
                    >
                                    ...
                                    {showFiltersMenu && (
                                        <div className="absolute right-0 top-full mt-2 w-80 bg-gradient-to-br from-[#111111] to-[#0a0a0a] border border-gray-800/50 rounded-lg shadow-xl z-50 p-4">
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                {(modalFilters.from || modalFilters.to || modalFilters.method || modalFilters.user || modalFilters.client) && (
                                                    <button
                        ...
                        <div className="px-6 py-4 border-b border-gray-800/50 bg-[#111111]/50">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="text-sm text-gray-300">
                                    Total: <span className="font-semibold text-[#f6b100]">{currency(modalCashClosure.grandTotal)}</span>
                        ...
                        <div className="flex-1 overflow-y-auto p-6">
                            <div className="flex justify-end mb-4">
                                <button
                                    onClick={() => downloadExcel(modalFilteredReports)}
                                    className="flex items-center gap-2 px-4 py-2 bg-[#1a1a1a] border border-gray-800/50 rounded-lg text-white hover:bg-[#262626] hover:border-[#f6b100]/50 transition-all"
                                >
                                    <Download className="w-4 h-4" />
                                    Exportar a Excel
